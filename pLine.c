...
int removeFromHead(pLine* list)
{
	checkNullLogExit(list, LIST_POINTER_ERROR);
	checkNullLogExit(list->head, LIST_POINTER_ERROR);

	int toReturn = list->head->lpatient; //save return value
	plnLine* temp = list->head->next; //save new list head
	free(list->head);
...
int peekList(const pLine* list)
{
	checkNullLogExit(list, LIST_POINTER_ERROR);
	checkNullLogExit(list->head, LIST_POINTER_ERROR);
	return list->head->lpatient;
}

void printList(const pLine* list, const char* delimiter)
{
	checkNullLogExit(list, LIST_POINTER_ERROR);
	...
	plnLine* temp = list->head;
	while (temp != LIST_END)
	{
		printf("%d", temp->lpatient);
		temp = temp->next;
		if (temp != NULL)
			printf(" %s ", delimiter);
	}
...
void addToTail(pLine* list, int data)
{
	checkNullLogExit(list, LIST_POINTER_ERROR);
	//create new list node
	plnLine* newTail = (plnLine*)calloc(1, sizeof(plnLine));
	checkNullLogExit(newTail, BAD_ALLOC);
	newTail->lpatient = data;
...
int isEmptyList(const pLine* list)
{
	checkNullLogExit(list, LIST_POINTER_ERROR);
...
void addToHead(pLine* list, int data)
{
	checkNullLogExit(list, LIST_POINTER_ERROR);
	//create new list node
	plnLine* newHead = (plnLine*)calloc(1, sizeof(plnLine));
	checkNullLogExit(newHead, BAD_ALLOC);
	newHead->lpatient = data;
	//case this is very first list element added to empty list
	if (list->head == EMPTY_LIST)
	{
...
int removeFromTail(pLine* list)
{
	checkNullLogExit(list, LIST_POINTER_ERROR);
	checkNullLogExit(list->head, LIST_POINTER_ERROR);
	//temp nodes to iterate over list and find the element previous to tail
	plnLine* current = list->head;
	plnLine* previous = NULL;
	...
	int toReturn = current->lpatient;//save return value
	free(current);
